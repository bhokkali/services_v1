...
public interface SchoolGradeRepository extends JpaRepository<SchoolGradeDao, Long> {
    ...
    @Query( value = "select count(*) from school_grades where academic_year_id=?1 and school_id=?2 ", nativeQuery = true)
    public Long findSchoolGradesCount(Long academicYearId, Long schoolId);
